...
— not yet safe for real Vault data. Durable storage would require attaching a
persistent data disk to the Server and pointing podman's storage
(`/var/lib/containers`) at it before `kind create cluster` runs; that is not
wired up here.
...
- **Edit the environment-specific values first** — these are placeholders for the
  ...
  - `server.yaml`: `namespace` (default `rh-vm-test1`)
...
**Cluster-side** (from the workstation, via `oc`): the Server, VMI, and Service
exist and expose the right ports.

```bash
oc get server,vmi,svc -n rh-vm-test1
oc get svc kind -n rh-vm-test1 -o jsonpath='{.spec.ports[*].port}'; echo   # expect 6443 and 30820
```

**In-VM** (in your Cirrus MFA SSH session): the kind cluster is healthy.
