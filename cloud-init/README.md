...
| Integration VM `Server`/Service name | `saw-integ` | `kubernetes/integrations-server.yml`, agent `inference_endpoint_url`, integration TLS SANs | Rename the Server and update the agent to call `https://<new-name>.<namespace>.svc.cluster.local:18083/v1`; regenerate the integration TLS certificate for the new DNS names. |
...
| Agent state PVC | `saw-agent-state-persist` | `kubernetes/agent-server.yml` | Update the `persistent-state` PVC claim name before creating `Server/saw-agent`. Existing data stays with the old PVC unless copied or recreated through the storage workflow. |
| Agent asset PVC | `saw-agent-assets-persist` | `kubernetes/agent-server.yml` | Update the `persistent-assets` PVC claim name before creating `Server/saw-agent`. This disk holds rootless container storage and OpenClaw sandbox assets. |
| Integrations persistence PVC | `saw-integ-persist` | `kubernetes/integrations-server.yml` | Update the `persistent-state` PVC claim name before creating `Server/saw-integ`. This disk holds the integration proxy state and provider key file. |
| Agent disk serials | `ONESTATE`, `ONEASSETS` | `kubernetes/agent-server.yml`, `ansible/agent.yml` | Change both the Cirrus mount serial and the Ansible disk discovery serial in the same commit. |
| Integrations disk serial | `TWOPERSIST` | `kubernetes/integrations-server.yml`, `ansible/site.yml` | Change both the Cirrus mount serial and the Ansible disk discovery serial in the same commit. |
...
certificate trust chain. An agent endpoint URL that says `saw-integ` must match
a TLS certificate that is valid for `saw-integ`, `saw-integ.<namespace>.svc`,
and `saw-integ.<namespace>.svc.cluster.local`.
...
deployment reaches OpenClaw through `saw-agent` and sends model traffic from
`saw-agent` to `saw-integ` over the internal `Service/saw-integ:18083` path.

## Experimental VM persistence

The `known-good/openclaw-persistent-pvcs` ref records the persistence baseline.
This branch keeps that persistence design and applies the OpenClaw SAW demo VM
names. It expects existing block PVCs in the same namespace as the `Server`
resources:
...
### Agent VM persistence

During `saw-agent` cloud-init:

- `saw-agent-state-persist` is attached with disk serial `ONESTATE`;
- `saw-agent-assets-persist` is attached with disk serial `ONEASSETS`;
- cloud-init leaves both persistent disks untouched so VM startup stays close to
  the known-good bootstrap path.

During `saw-agent` provisioning:
...
### Integrations VM persistence

`saw-integ` expects an existing block PVC named `saw-integ-persist` in the same
namespace as `Server/saw-integ`.

During `saw-integ` cloud-init:

- the `saw-integ-persist` PVC is attached with disk serial `TWOPERSIST`;
...
material, is still reconciled from `Secret/saw-integ-vars` so `saw-agent` and
`saw-integ` stay aligned when the internal bearer or certificates are
