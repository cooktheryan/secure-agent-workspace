...
stable_runtime_block="$(awk '/exec \/usr\/local\/bin\/openshell sandbox exec/{in_block=1} in_block{print} in_block && /nohup openclaw gateway run/{seen_gateway=1} seen_gateway && /<\/dev\/null/{exit}' "$agent_playbook")"
grep -Fq -- '--env NODE_ICU_DATA=/opt/openclaw/node_modules/full-icu' <<<"$stable_runtime_block"
grep -Fq -- '--env NODE_DISABLE_COMPILE_CACHE=1' <<<"$stable_runtime_block"
grep -Fq 'export NODE_ICU_DATA=/opt/openclaw/node_modules/full-icu' <<<"$stable_runtime_block"
grep -Fq 'export NODE_DISABLE_COMPILE_CACHE=1' <<<"$stable_runtime_block"
